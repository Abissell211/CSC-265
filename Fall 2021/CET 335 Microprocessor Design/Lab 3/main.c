...
int main (void) {
  ...
  while(1) {
    printf("\nGive length: "); scanf("%ld", &length); printf(" m"); // Get input
    printf("\nGive width: ");  scanf("%ld", &width); printf(" m");  // Get input
    area = Calc_Area(length, width);									 						  // Function call for calculating the area
    printf("\nArea of the room = %ld m^2 \n",  area);			 				  // Display of calculated area
  }
}
