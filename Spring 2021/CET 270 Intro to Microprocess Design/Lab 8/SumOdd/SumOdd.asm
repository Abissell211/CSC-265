        TTL     "Lab 8 - SumOdd Program"

******************************************************************
* 1) HEADER block - overall info about the program

* Project name:   SumOdd - Lab08
* Author(s):      Andrew D. Bissell
* Date:           03/25/2021
* Description:
*   This program adds all of the Odd numbers in an array and
...
ChkAdd  LDAA    0,X        ; get the number pointed to by X.
        ...
        BEQ     ChkDONE    ; if EVEN (LSB = 0), ignore and go check 
;                            next number.
; OR ! ------------------------------------------------------------        
;       LSRA               ; Shift LSB into the C-flag.
;       BCC     ChkDONE    ; If Carry = 0 (EVEN Number), 
;                            skip to check if done ->
; -----------------------------------------------------------------

        LDAA    SUM        ; Otherwise, the number is ODD (LSB = 1),
...
ChkDONE INX                ; Move X to next number in array.
        ...
        ANDA    #$0F       ; ANDA with $04 to clear left bit.
        ORAA    #$30       ; ORAA with $34 to load bit with ASCII CHAR
        JSR     OUTA       ; Call BuFFALO rtn to output ACC A.
...
OUTPUT  FCB     "The sum of ODD numbers in the array are: $",EOT
NEWLINE FCB     " ",CR,EOT
******************************************************************
